...
class Pulseira:
    ...
    def remove_pessoa_conhecida(self, x, y):
        pessoa_encontrada = None
        for pessoa in self.pessoas_conhecidas:
            if pessoa.x == x and pessoa.y == y:
                pessoa_encontrada = pessoa
                break

        if pessoa_encontrada:
            self.pessoas_conhecidas.remove(pessoa_encontrada)
            self.app.remove_pessoa_conhecida_square(pessoa_encontrada)
    ...
    def load_pessoas_cadastradas(self):
        if os.path.exists("contatos.txt"):
            with open("contatos.txt", "r") as file:
                for line in file:
                    nome = line.strip()
                    self.add_pessoa_conhecida(nome, 250, 250)
...
class App:
    def __init__(self, root):
        ...
        self.pulseira.create_contatos_file_if_not_exists()
    ...
    def on_click(self, event):
        ...
        for pessoa in self.pulseira.pessoas_conhecidas:
            if (pessoa.x - 10) <= x <= (pessoa.x + 10) and (pessoa.y - 10) <= y <= (pessoa.y + 10):
                self.selected_pessoa = pessoa
                self.pulseira.remove_pessoa_conhecida(x, y)
                break

        if (self.pulseira.pessoa_nao_cadastrada.x - 10) <= x <= (self.pulseira.pessoa_nao_cadastrada.x + 10) and (self.pulseira.pessoa_nao_cadastrada.y - 10) <= y <= (self.pulseira.pessoa_nao_cadastrada.y + 10):
    ...
    def excluir(self):
        ...
        if nome:
            ...
            if resposta:
                self.pulseira.remove_pessoa_conhecida(250, 250)
            self.entry.delete(0, tk.END)
