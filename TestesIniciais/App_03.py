...
class Pulseira:
    ...
    def add_pessoa_conhecida(self, nome, x, y):
        ...
        self.pulseira.pessoas_conhecidas.append(pessoa)
        self.pessoas_conhecidas_squares.append(self.create_pessoa_conhecida_square(pessoa))  # Adicione esta linha
        self.app.save_pessoas_cadastradas()


    def remove_pessoa_conhecida(self, nome):
        ...
        if pessoa_encontrada:
            self.pulseira.pessoas_conhecidas.remove(pessoa_encontrada)
            self.app.update_message(f"Pessoa {pessoa_encontrada.nome} removida e tornada desconhecida!")

            # Remove o quadrado correspondente
            index = self.pessoas_conhecidas_squares.index(self.pessoas_conhecidas_squares[pessoa_encontrada])
            square = self.pessoas_conhecidas_squares.pop(index)
            self.canvas.delete(square)

            self.app.save_pessoas_cadastradas()  # Atualiza o arquivo

        else:
            self.app.update_message(f"Pessoa {nome} não encontrada.")
    ...
    def load_pessoas_cadastradas(self):
        if os.path.exists("contatos.txt"):
            with open("contatos.txt", "r") as file:
                for line in file:
                    nome = line.strip()  # Lê apenas o nome
                    self.add_pessoa_conhecida(nome, 250, 250)  # Posição padrão
...
class App:
    ...
    def cadastrar(self):
        nome = self.entry.get()
        if nome:
            self.pulseira.add_pessoa_conhecida(nome, 250, 250)  # Adiciona com posição padrão
            self.entry.delete(0, tk.END)  # Limpa o campo de texto após cadastrar

    def excluir(self):
        nome = self.entry.get()
        if nome:
            resposta = messagebox.askyesno("Excluir Pessoa", f"Deseja realmente excluir a pessoa {nome}?")
            if resposta:
                self.pulseira.remove_pessoa_conhecida(nome)
            self.entry.delete(0, tk.END)  # Limpa o campo de texto após excluir

    def save_pessoas_cadastradas(self):
        with open("contatos.txt", "w") as file:
            for pessoa in self.pulseira.pessoas_conhecidas:
                file.write(f"{pessoa.nome}\n")
    ...
    def load_pessoas_cadastradas(self):
        if os.path.exists("contatos.txt"):
            with open("contatos.txt", "r") as file:
                for line in file:
                    nome = line.strip()  # Lê apenas o nome
                    self.pulseira.add_pessoa_conhecida(nome, 250, 250)
